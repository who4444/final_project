...
class UserManager(LibraryDB):
    ...
    def add_registered_info(self, Name, Gender, User_name, Password_hash, DOB, Address, Phone_num, Interests):
        hashed_password = bcrypt.hashpw(Password_hash.encode('utf-8'), bcrypt.gensalt()).decode('utf-8')
        query="""INSERT INTO Reader (Name, Gender, User_Name, Password_Hash, DOB, Address, Phone_Number, Interests)
                    VALUES (?,?,?,?,?,?,?,?)"""
        self.cursor.execute(query,(Name, Gender, User_name, hashed_password, DOB, Address, Phone_num, Interests))
        self.conn.commit()
    ...
    def get_profile(self):
        self.cursor.execute("SELECT User_Name, Name,Gender, Address, DOB, Phone_Number, Interests FROM Reader WHERE Reader_ID = ?", (self.user_id,))
        return self.cursor.fetchone()

    def update_profile(self, User_name: str, Name: str,
                       Address: str, Phone_num: str, Gender: str,
                       DOB: str, Interests: str) -> None:
        """Update current user's profile with DOB as a date."""
        self.cursor.execute(
            """UPDATE Reader 
            ...
            (User_name, Name, Address, Phone_num,
             Gender, DOB, Interests, self.user_id)
        )
        self.conn.commit()

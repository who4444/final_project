...
class AdminManager(LibraryDB):
    ...
    def get_users(self):
        ...
        self.cursor.execute("""
            ...
            GROUP BY c.Reader_ID, c.Name, c.DOB, c.Membership_Status
        """)
        data = self.cursor.fetchall()
        column_names = ['ID', 'Name', 'DOB', 'Membership Status', 'Books Borrowed']
...
WHERE
    ...
    def get_categories(self):
        self.cursor.execute("""
            ...
            g.Genre_ID,
            g.Genre,
            MIN(b.Title) AS Representative_Book,
            COALESCE(MAX(borrow_counts.borrow_count), 0) AS Max_Borrow_Count,
        ...
        LEFT JOIN
            (
                ...
                GROUP BY
                    bb.book_id
            ) AS borrow_counts ON b.Book_ID = borrow_counts.book_id
        GROUP BY
            g.Genre_ID, g.Genre;""")
        data = self.cursor.fetchall()
        column_names = [desc[0] for desc in self.cursor.description]
        return column_names, data
    def is_genre_exists(self, genre):
        self.cursor.execute("SELECT * FROM Genre WHERE Genre = ?", (genre,))
        result = self.cursor.fetchone()
        if result is not None:
            return result[0] > 0
        else:
            return False  # Or handle the empty result case appropriately
    ...
    def add_general_notification(self, message):
        try:
            ...
            SELECT Reader_ID, ?, GETDATE(), 1 FROM Reader;
            """

...
class UserManager(LibraryDB):
    ...
    def get_profile(self):
        self.cursor.execute("SELECT User_Name, Name, Address, Age, Phone_Number, Interests FROM Reader WHERE Reader_ID = ?", (self.user_id,))
        return self.cursor.fetchone()
    def update_profile(self, User_name: str, Password: str, Name: str, 
                      Address: str, Phone_num: str, Gender: str, 
                      Age: int, Interests: str) -> None:
        """Update current user's profile."""
        # Get current password hash
        self.cursor.execute(
            "SELECT Password_Hash FROM Reader WHERE Reader_ID = ?",
            (self.user_id,)
        )
        ...
        hashed_password = (bcrypt.hashpw(Password.encode('utf-8'), bcrypt.gensalt()).decode('utf-8') 
                         if Password.strip() else current_hashed_password)

        self.cursor.execute(
            """UPDATE Reader 
               SET User_Name = ?, Password_Hash = ?, Name = ?, 
                   Address = ?, Phone_Number = ?, Gender = ?, 
                   Age = ?, Interests = ? 
               WHERE Reader_ID = ?""",
            (User_name, hashed_password, Name, Address, Phone_num, 
    ...
    def get_my_inventory(self) -> tuple:
        ...
        self.cursor.execute("""
            SELECT 
                ...
                CASE 
                    WHEN BorrowedBook.Returned = 1 THEN 'Returned'
                    WHEN BorrowedBook.Return_Date >= GETDATE() THEN 'Not Returned'
                    ELSE 'Overdue'
                END as Status
            FROM BorrowedBook 
    ...
    def get_review(self, book_id: int) -> tuple[list, list]:
        ...
        self.cursor.execute("""
            SELECT 
                ...
                c.User_Name,
                br.Review_Text,
                FORMAT(br.Review_Date, 'yyyy-MM-dd HH:mm') as Review_Date
            FROM BookReview br
    ...
    def get_notifications(self):
        self.cursor.execute("SELECT Message, Timestamp FROM Notification WHERE Reader_ID = ? ORDER BY Timestamp DESC",
                            (self.user_id,))
        return self.cursor.fetchall()
